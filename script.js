...
document.querySelector('.logo-link').addEventListener('click', function (e) {
    ...
    if (window.location.pathname.endsWith('index.html')) {
        e.preventDefault();
        const targetId = this.getAttribute('href').split('#')[1]; // Получаем якорь
        const targetElement = document.getElementById(targetId);
        if (targetElement) {
            const offset = 150; // Отступ от верха
    ...
    // Если не на главной странице, переход произойдёт автоматически
});
